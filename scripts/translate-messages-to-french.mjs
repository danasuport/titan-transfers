/**
 * Translate the UI message catalog en.json → fr.json via OpenAI.
 *
 ...
 *
 * Usage:
 *   node scripts/translate-messages-to-french.mjs [--force] [--model=gpt-4o-mini] [--batch=40]
 *
 * Reads OPENAI_API_KEY from .env.local. Writes src/messages/fr.json.
 * Without --force, keys already present in an existing fr.json are kept as-is
...
const OUT = 'src/messages/fr.json'
...
const SYSTEM_PROMPT = `You are a professional translator localizing the UI of a private-transfer booking website into French (fr-FR).

You receive a JSON object mapping opaque keys to English UI strings. Return a JSON object with the SAME keys, where every value is the French translation.
...
- Use natural, concise, marketing-friendly standard French appropriate for buttons, labels, headings and short paragraphs. Use the formal address (vous) when addressing the user.
- Use correct French accents and typography (é, è, ê, à, ç, ï, ô…) freely in the visible UI text — these rules only affect URL slugs elsewhere, not this catalog.
  ...
  const res = await fetch('https://api.openai.com/v1/chat/completions', {
    ...
    body: JSON.stringify({
      ...
      messages: [
        ...
        { role: 'user', content: `Translate these UI strings to French:\n${JSON.stringify(batchObj)}` },
      ],
      temperature: 0.3,
      max_tokens: 8000,
    }),
...
async function run() {
  ...
  for (let i = 0; i < todo.length; i += BATCH) {
    ...
    try {
      ...
      for (const k of slice) setDeep(result, k, flatEn[k])
    }
