 ...
 *
 * Reads ANTHROPIC_API_KEY from .env.local. Writes to Sanity (see scripts/lib/sanity-client.mjs).
 *
  ...
  try {
    ...
    await client
      ...
      .set({ 'translations.fr': {
        ...
        ...(doc.translations?.fr || {}), ...fr, ...(doc.translations?.fr?.slug?.current ? { slug: doc.translations.fr.slug } : {}),
      } })
      .commit()
