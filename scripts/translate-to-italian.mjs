...
const SYSTEM_PROMPT = `You are a professional translator specializing in Italian for the travel and transportation industry, targeting travellers in Italy.

RULES:
- Translate to natural, fluent, marketing-friendly Italian (it-IT) — not literal/robotic translation.
- Keep prices, distances, durations and ratings with Western numerals (0-9).
- Keep brand names ("Titan Transfers"), airport codes and airport names in their original form. For places, use the standard Italian exonym when one exists (e.g. London → Londra, Paris → Parigi, Spain → Spagna, Munich → Monaco di Baviera), otherwise keep the local name.
- Keep email addresses and URLs in their original form.
- Use the formal but warm register typical of Italian travel websites.
...
function buildPrompt(doc) {
  ...
  return `Translate this ${docKind} from English to Italian.

Input JSON (English source):
${JSON.stringify(payload, null, 2)}

Return the translated JSON with the SAME shape. Every text field must be in Italian. Do not include a "slug" field.`
}
...
async function translateDoc(doc) {
  const existing = doc.translations?.it
  ...
  // Build the translations.it object for Sanity. Don't overwrite an existing slug.
  const it = {}
  if (translated.title) it.title = translated.title
  // Italian slug derived from the translated title (accents stripped).
...
async function processDoc(doc) {
  ...
  try {
    ...
    if (DRY_RUN) {
      console.log(`  [DRY RUN] would patch translations.it:`, Object.keys(it))
      return
    }
    ...
    await client
      ...
      .set({ 'translations.it': it })
      .commit()

    console.log(`  ✓ Patched translations.it (${Object.keys(it).join(', ')})`)
  } catch (e) {
