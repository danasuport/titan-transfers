/**
 * Translate Sanity content to Italian via OpenAI Chat Completions.
 *
 * Mirrors translate-to-arabic.mjs: reads every document of the supported types,
 ...
 * it to the model, and writes the result into translations.it on the same doc.
 *
 * Unlike Arabic, Italian uses the Latin alphabet, so slugs are derived from the
 * translated Italian title (accents stripped), not transliterated.
 *
 * Usage:
 *   node scripts/translate-to-italian.mjs \
 *     [--type=city,airport,...] [--limit=10] [--force] [--dry-run] [--model=claude-opus-5]
 *
 * Reads ANTHROPIC_API_KEY from .env.local. Writes to Sanity project 6iu2za90
 * (see scripts/lib/sanity-client.mjs).
 *
 * Skips documents that already have translations.it.title (idempotent), unless
...
const SYSTEM_PROMPT = `You are a professional translator specializing in Spanish for the travel and transportation industry, targeting travellers in Spain and Latin America.

RULES:
- Translate to natural, fluent, marketing-friendly Spanish (es-ES) — not literal/robotic translation. Avoid translationese; write the way a Spanish copywriter would.
- Keep prices, distances, durations and ratings with Western numerals (0-9).
- Keep brand names ("Titan Transfers"), airport codes and airport names in their original form. For places, use the standard Spanish exonym when one exists (e.g. London → Londres, Munich → Múnich, Florence → Florencia), otherwise keep the local name.
- Keep email addresses and URLs in their original form.
- Use the "usted"-neutral but warm register typical of Spanish travel websites; avoid being stiff.
...
function buildPrompt(doc) {
  ...
  return `Translate this ${docKind} from English to Spanish.

Input JSON (English source):
${JSON.stringify(payload, null, 2)}

Return the translated JSON with the SAME shape. Every text field must be in Spanish. Do not include a "slug" field.`
}
...
async function translateDoc(doc) {
  const existing = doc.translations?.es
  ...
  // Build the translations.es object for Sanity. Don't overwrite an existing slug.
  const it = {}
  if (translated.title) it.title = translated.title
  // Spanish slug derived from the translated title (accents stripped).
  const slugSource = translated.title || doc.title
  if (slugSource && !existing?.slug?.current) {
    const current = slugify(slugSource)
    if (current) it.slug = { _type: 'slug', current }
  }
  if (translated.seoTitle) it.seoTitle = translated.seoTitle
  if (translated.seoDescription) it.seoDescription = translated.seoDescription
  if (translated.excerpt) it.excerpt = translated.excerpt
  if (translated.description) it.description = flattenedToPortableText(translated.description)
  if (translated.content) it.content = flattenedToPortableText(translated.content)
  if (translated.contentSections) {
    it.contentSections = translated.contentSections.map(s => ({
      _key: randomUUID().slice(0, 12),
      title: s.title || '',
...
async function processDoc(doc) {
  ...
  try {
    const it = await translateDoc(doc)
    if (!it) return

    if (DRY_RUN) {
      console.log(`  [DRY RUN] would patch translations.es:`, Object.keys(it))
      return
    }
    ...
    await client
      ...
      .set({ 'translations.es': {
        ...
        ...(doc.translations?.es || {}), ...it, ...(doc.translations?.es?.slug?.current ? { slug: doc.translations.es.slug } : {}),
      } })
      .commit()
...
async function run() {
  ...
  if (FORCE) console.log('Force: overwriting existing translations.it')
