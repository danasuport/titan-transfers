/**
 * Add German ('de') variants to inline pick(locale, { en, es, ar, it }) calls.
 *
 ...
 * which is safe). Existing en/es/ar/it entries are never modified — we only
 * insert a new `de` entry, translated from the English (en) source.
 *
 * Usage:
 *   node scripts/translate-pick-inline-german.mjs            # dry-run: report only
 *   node scripts/translate-pick-inline-german.mjs --apply    # write changes
 *   [--model=gpt-4o-mini]
 *
...
for (const file of files) {
  ...
  while ((m = re.exec(src))) {
    ...
    const onlyLocaleKeys = keys.every(k => ['en', 'es', 'ar', 'it', 'de'].includes(k))
    if (!allStrings || !onlyLocaleKeys || keys.includes('de')) { stats.skipped++; continue }
...
const SYSTEM = `You translate short UI strings from English to German (de-DE) for a private-transfer booking website.
Return a JSON object mapping each id to its German translation.
RULES:
...
- Natural, concise, marketing-friendly standard German (Hochdeutsch), formal address (Sie). Umlauts (ä/ö/ü/ß) are fine here. Do not add quotes around values.
...
async function translateBatch(map) {
  const res = await fetch('https://api.openai.com/v1/chat/completions', {
    ...
    body: JSON.stringify({ model: MODEL, response_format: { type: 'json_object' }, temperature: 0.3, max_tokens: 8000,
      messages: [{ role: 'system', content: SYSTEM }, { role: 'user', content: 'Translate to German:\n' + JSON.stringify(map) }] }),
  })
...
for (let i = 0; i < jobs.length; i += BATCH) {
  ...
  slice.forEach((j, k) => { j.deText = out[String(i + k)] })
  console.log('done')
...
for (const [file, fjobs] of Object.entries(byFile)) {
  ...
  for (const j of fjobs) {
    if (typeof j.deText !== 'string' || !j.deText) continue
    ...
    if (j.multiline) {
      ...
      const insertion = `${j.indent}de: ${itLiteral},\n`
      ...
      src = before + `, de: ${itLiteral} ` + src.slice(j.closeIdx)
    }
  }
  writeFileSync(file, src)
  console.log(`  ✓ ${file} (+${fjobs.filter(j => j.deText).length})`)
}
