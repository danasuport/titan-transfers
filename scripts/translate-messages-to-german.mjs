/**
 * Translate the UI message catalog en.json → de.json via OpenAI.
 *
 ...
 *
 * Usage:
 *   node scripts/translate-messages-to-german.mjs [--force] [--model=gpt-4o-mini] [--batch=40]
 *
 * Reads OPENAI_API_KEY from .env.local. Writes src/messages/de.json.
 * Without --force, keys already present in an existing de.json are kept as-is
...
const OUT = 'src/messages/de.json'
...
const SYSTEM_PROMPT = `You are a professional translator localizing the UI of a private-transfer booking website into German (de-DE).

You receive a JSON object mapping opaque keys to English UI strings. Return a JSON object with the SAME keys, where every value is the German translation.
...
- Use natural, concise, marketing-friendly standard German (Hochdeutsch) appropriate for buttons, labels, headings and short paragraphs. Use the formal address (Sie) when addressing the user.
- Use standard German umlauts (ä, ö, ü, ß) freely in the visible UI text — these rules only affect URL slugs elsewhere, not this catalog.
  ...
  const res = await fetch('https://api.openai.com/v1/chat/completions', {
    ...
    body: JSON.stringify({
      ...
      messages: [
        ...
        { role: 'user', content: `Translate these UI strings to German:\n${JSON.stringify(batchObj)}` },
      ],
      temperature: 0.3,
      max_tokens: 8000,
    }),
