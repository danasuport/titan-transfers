    ...
    <div style={{ padding: '1.5rem 0 1rem' }}>
      ...
      {type === 'airports' && (
        <div>
          ...
          <Footer href="/airports/" label={pick(locale, { en: 'Browse all airports', es: 'Ver todos los aeropuertos', ar: 'تصفّح كل المطارات', it: 'Vedi tutti gli aeroporti', de: 'Alle Flughäfen durchsuchen', fr: 'Parcourir tous les aéroports' })} onClose={onClose} />
        </div>
      )}
      ...
      {type === 'cities' && (
        <div>
          ...
          <Footer href="/cities/" label={pick(locale, { en: 'Browse all cities', es: 'Ver todas las ciudades', ar: 'تصفّح كل المدن', it: 'Vedi tutte le città', de: 'Alle Städte durchsuchen', fr: 'Parcourir toutes les villes' })} onClose={onClose} />
        </div>
      )}
      ...
      {type === 'countries' && (
        <div>
                ...
                <div>
                  ...
                  {(c.airportCount || c.cityCount) ? (
                    <div style={{ fontSize: '0.7rem', color: 'inherit', opacity: 0.7 }}>
                      {c.airportCount ? `${c.airportCount} ${pick(locale, { en: 'airports', es: 'aeropuertos', ar: 'مطار', it: 'aeroporti', de: 'Flughäfen', fr: 'aéroports' })}` : ''}
                      {c.airportCount && c.cityCount ? ' · ' : ''}
                      {c.cityCount ? `${c.cityCount} ${pick(locale, { en: 'cities', es: 'ciudades', ar: 'مدينة', it: 'città', de: 'Städte', fr: 'villes' })}` : ''}
                    </div>
                  ) : null}
                </div>
              </HoverItem>
          ...
          <Footer href="/countries/" label={pick(locale, { en: 'Browse all countries', es: 'Ver todos los países', ar: 'تصفّح كل الدول', it: 'Vedi tutti i paesi', de: 'Alle Länder durchsuchen', fr: 'Parcourir tous les pays' })} onClose={onClose} />
        </div>
      )}
...
function MegaMenuMobile({ type, onClose, locale, data, countries, airportsByCountry, citiesByCountry, title, airportHref, cityHref, countryHref }: {
  ...
  return (
    <div style={{ padding: '0.5rem 0' }}>
      {!data && <div style={{ padding: '1rem 0', color: '#64748b', fontSize: '0.85rem' }}>{pick(locale, { en: 'Loading...', es: 'Cargando...', ar: 'جارٍ التحميل...', it: 'Caricamento...', de: 'Wird geladen...', fr: 'Chargement...' })}</div>}
        ...
        <>
          ...
          <div style={{ paddingTop: '1rem' }}>
            <Link href={(type === 'airports' ? '/airports/' : '/cities/') as never} onClick={onClose}
              ...
              {type === 'airports'
                ? pick(locale, { en: 'Browse all airports →', es: 'Ver todos los aeropuertos →', ar: '← تصفّح كل المطارات', it: 'Vedi tutti gli aeroporti →', de: 'Alle Flughäfen durchsuchen →', fr: 'Parcourir tous les aéroports →' })
                : pick(locale, { en: 'Browse all cities →', es: 'Ver todas las ciudades →', ar: '← تصفّح كل المدن', it: 'Vedi tutte le città →', de: 'Alle Städte durchsuchen →', fr: 'Parcourir toutes les villes →' })}
            </Link>
          </div>
        </>
      )}
      ...
      {type === 'countries' && data && (
        <>
                ...
                <div>
                  ...
                  {(c.airportCount || c.cityCount) && (
                    <div style={{ fontSize: '0.65rem', color: '#64748b' }}>
                      {[
                        c.airportCount && `${c.airportCount} ${pick(locale, { en: 'airports', es: 'aerop.', ar: 'مطار', it: 'aeroporto', de: 'Flughäfen', fr: 'aéroports' })}`,
                        c.cityCount && `${c.cityCount} ${pick(locale, { en: 'cities', es: 'ciud.', ar: 'مدينة', it: 'città', de: 'Städte', fr: 'villes' })}`,
                      ].filter(Boolean).join(' · ')}
                    </div>
                  )}
                </div>
          ...
          <Link href={'/countries/' as never} onClick={onClose}
            style={{ display: 'block', textAlign: 'center', background: '#242426', color: '#ffffff', padding: '0.6rem 1.25rem', fontWeight: 700, fontSize: '0.8rem', textDecoration: 'none', fontFamily: 'inherit', width: '100%', boxSizing: 'border-box' }}>
            {pick(locale, { en: 'Browse all countries →', es: 'Ver todos los países →', ar: '← تصفّح كل الدول', it: 'Vedi tutti i paesi →', de: 'Alle Länder durchsuchen →', fr: 'Parcourir tous les pays →' })}
          </Link>
        </>
      )}
    </div>
...
function Empty({ locale }: { locale: string }) {
  return <p style={{ fontSize: '0.875rem', color: '#64748b', padding: '1rem 0' }}>{pick(locale, { en: 'No results', es: 'Sin resultados', ar: 'لا توجد نتائج', it: 'Nessun risultato', de: 'Keine Ergebnisse', fr: 'Aucun résultat' })}</p>
}
