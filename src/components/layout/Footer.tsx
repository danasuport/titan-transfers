...
const headingStyle = { color: '#242426', fontSize: '1.1rem', letterSpacing: '0.01em' }
...
function AccordionCol({ title, children }: { title: string; children: React.ReactNode }) {
  ...
  return (
    <div className="footer-accordion-col">
      ...
      <div className={`${russoOne.className} footer-col-heading-desktop`} style={{ ...headingStyle, marginBottom: '1.25rem' }}>
        {title}
      </div>
      ...
      <button
        ...
        onClick={() => setOpen(o => !o)}
        style={{ background: 'none', border: 'none', cursor: 'pointer', width: '100%', display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '0.85rem 0', borderBottom: '1px solid #e2e8f0' }}
      >
        <span className={russoOne.className} style={{ ...headingStyle, marginBottom: 0 }}>{title}</span>
        <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="#475569" strokeWidth="2.5"
          style={{ transform: open ? 'rotate(180deg)' : 'rotate(0deg)', transition: 'transform 0.2s', flexShrink: 0 }}>
          <path strokeLinecap="round" strokeLinejoin="round" d="M19 9l-7 7-7-7" />
        </svg>
      </button>
      {/* Desktop: always shown / Mobile: collapsible */}
      <div className={`footer-col-content${open ? ' open' : ''}`}>
        {children}
      </div>
    </div>
  )
...
export function Footer() {
  ...
  return (
    <footer style={{ background: '#F8FAF0', color: '#475569' }}>
        ...
        <div className="footer-main-grid" style={{ display: 'grid', gridTemplateColumns: '2fr 1fr 1fr 1fr 0.9fr', gap: '3rem', marginBottom: '3rem' }}>
          ...
          <div className="footer-brand-col">
            ...
            <div style={{ display: 'flex', gap: '0.75rem' }}>
              {[
                { label: 'Instagram', href: 'https://www.instagram.com/titan_transfers/', path: 'M12 2.163c3.204 0 3.584.012 4.85.07 3.252.148 4.771 1.691 4.919 4.919.058 1.265.069 1.645.069 4.849 0 3.205-.012 3.584-.069 4.849-.149 3.225-1.664 4.771-4.919 4.919-1.266.058-1.644.07-4.85.07-3.204 0-3.584-.012-4.849-.07-3.26-.149-4.771-1.699-4.919-4.92-.058-1.265-.07-1.644-.07-4.849 0-3.204.013-3.583.07-4.849.149-3.227 1.664-4.771 4.919-4.919 1.266-.057 1.645-.069 4.849-.069zm0-2.163c-3.259 0-3.667.014-4.947.072-4.358.2-6.78 2.618-6.98 6.98-.059 1.281-.073 1.689-.073 4.948 0 3.259.014 3.668.072 4.948.2 4.358 2.618 6.78 6.98 6.98 1.281.058 1.689.072 4.948.072 3.259 0 3.668-.014 4.948-.072 4.354-.2 6.782-2.618 6.979-6.98.059-1.28.073-1.689.073-4.948 0-3.259-.014-3.667-.072-4.947-.196-4.354-2.617-6.78-6.979-6.98-1.281-.059-1.69-.073-4.949-.073zm0 5.838c-3.403 0-6.162 2.759-6.162 6.162s2.759 6.163 6.162 6.163 6.162-2.759 6.162-6.163c0-3.403-2.759-6.162-6.162-6.162zm0 10.162c-2.209 0-4-1.79-4-4 0-2.209 1.791-4 4-4s4 1.791 4 4c0 2.21-1.791 4-4 4zm6.406-11.845c-.796 0-1.441.645-1.441 1.44s.645 1.44 1.441 1.44c.795 0 1.439-.645 1.439-1.44s-.644-1.44-1.439-1.44z' },
                { label: 'Facebook', href: 'https://www.facebook.com/profile.php?id=61579031776370', path: 'M24 12.073c0-6.627-5.373-12-12-12s-12 5.373-12 12c0 5.99 4.388 10.954 10.125 11.854v-8.385H7.078v-3.47h3.047V9.43c0-3.007 1.792-4.669 4.533-4.669 1.312 0 2.686.235 2.686.235v2.953H15.83c-1.491 0-1.956.925-1.956 1.874v2.25h3.328l-.532 3.47h-2.796v8.385C19.612 23.027 24 18.062 24 12.073z' },
                { label: 'LinkedIn', href: 'https://www.linkedin.com/company/titan-transfers', path: 'M20.447 20.452h-3.554v-5.569c0-1.328-.027-3.037-1.852-3.037-1.853 0-2.136 1.445-2.136 2.939v5.667H9.351V9h3.414v1.561h.046c.477-.9 1.637-1.85 3.37-1.85 3.601 0 4.267 2.37 4.267 5.455v6.286zM5.337 7.433c-1.144 0-2.063-.926-2.063-2.065 0-1.138.92-2.063 2.063-2.063 1.14 0 2.064.925 2.064 2.063 0 1.139-.925 2.065-2.064 2.065zm1.782 13.019H3.555V9h3.564v11.452zM22.225 0H1.771C.792 0 0 .774 0 1.729v20.542C0 23.227.792 24 1.771 24h20.451C23.2 24 24 23.227 24 22.271V1.729C24 .774 23.2 0 22.222 0h.003z' },
              ].map(({ label, href, path }) => (
                <a key={label} href={href} target="_blank" rel="noopener noreferrer" aria-label={label} style={{ width: '42px', height: '42px', background: '#242426', display: 'flex', alignItems: 'center', justifyContent: 'center', transform: 'skewX(-12deg)', transition: 'background 0.2s' }}
                  onMouseEnter={e => (e.currentTarget.style.background = '#8BAA1D')}
                  onMouseLeave={e => (e.currentTarget.style.background = '#242426')}
                >
                  <svg style={{ transform: 'skewX(12deg)' }} width="19" height="19" viewBox="0 0 24 24" fill="#ffffff"><path d={path} /></svg>
                </a>
              ))}
            </div>
          </div>
          ...
          {/* Quick links */}
          <AccordionCol title={t('quickLinks')}>
          ...
          {/* Contact */}
          <AccordionCol title={pick(locale, { en: 'Contact info', es: 'Contacto', ar: 'بيانات التواصل', it: 'Contatto', de: 'Kontaktinformationen', fr: 'Informations de contact' })}>
            <ul style={{ listStyle: 'none', padding: 0, margin: 0, display: 'flex', flexDirection: 'column', gap: '0.6rem' }}>
              {[
                ...
                { href: `/${locale}/${getLocalizedPath('contact', locale as Locale)}/`, label: pick(locale, { en: 'Contact form', es: 'Formulario de contacto', ar: 'نموذج التواصل', it: 'Modulo di contatto', de: 'Kontaktformular', fr: 'Formulaire de contact' }) },
              ].map(item => (
                <li key={item.href}>
          ...
          {/* Reviews */}
          <AccordionCol title={pick(locale, { en: 'Reviews', es: 'Opiniones', ar: 'التقييمات', it: 'Recensioni', de: 'Bewertungen', fr: 'Avis' })}>
              ...
              {(() => {
                ...
                >
                  <div style={{ display: 'flex', alignItems: 'center', gap: '0.4rem' }}>
                    {icon}
                    <span style={{ fontWeight: 700, fontSize: '0.85rem', color: '#242426' }}>{name}</span>
                  </div>
                  <div style={{ display: 'flex', gap: '2px' }}>
                    {[...Array(5)].map((_, i) => <svg key={i} width="13" height="13" viewBox="0 0 24 24" fill={starColor}><path d="M12 0l3.09 9.26H24l-7.85 5.7 3.09 9.26L12 18.52l-7.24 5.7 3.09-9.26L0 9.26h8.91z"/></svg>)}
                  </div>
        ...
        <div style={{ paddingTop: '2rem', display: 'flex', justifyContent: 'space-between', alignItems: 'center', flexWrap: 'wrap', gap: '1rem' }}>
          ...
          <div style={{ display: 'flex', gap: '1.5rem', flexWrap: 'wrap' }}>
            {[
              ...
              { href: '/sitemap-page/', label: pick(locale, { en: 'Sitemap', es: 'Mapa del sitio', ar: 'خريطة الموقع', it: 'Mappa del sito', de: 'Sitemap', fr: 'Plan du site' }) },
            ].map(item => (
      ...
      <style>{`
        /* Desktop: acordeón oculto, layout normal */
        .footer-col-toggle { display: none !important; }
        .footer-col-content { display: block !important; padding-top: 0; }
        .footer-col-heading-desktop { display: block; }
        ...
        @media (max-width: 768px) {
          ...
          .footer-brand-col { padding-bottom: 2rem; border-bottom: 1px solid #e2e8f0; margin-bottom: 0.5rem; }
          .footer-col-heading-desktop { display: none !important; }
          .footer-col-toggle { display: flex !important; }
          .footer-col-content {
            display: none !important;
            padding: 0.75rem 0 1rem;
          }
          .footer-col-content.open { display: block !important; }
          .footer-accordion-col { border-bottom: 1px solid #e2e8f0; }
        }
      `}</style>
    </footer>
  )
