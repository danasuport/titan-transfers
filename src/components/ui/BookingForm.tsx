...
export function BookingForm() {
  ...
  return (
    <>
      <form onSubmit={handleSubmit} style={{ background: '#ffffff', border: '2px solid #8BAA1D', padding: '1.5rem 2rem 1.25rem' }}>
        ...
        <div style={{ display: 'flex', gap: '0.75rem', alignItems: 'stretch' }}>
          ...
          <div style={{ ...fieldStyle, flex: '0 0 auto' }}>
            ...
            <select value={pax} onChange={e => setPax(Number(e.target.value))} style={{ border: 'none', outline: 'none', background: 'transparent', fontSize: '0.875rem', color: '#242426', fontFamily: 'inherit', cursor: 'pointer' }}>
              {[...Array(16)].map((_, i) => <option key={i+1} value={i+1}>{i+1} {es ? (i === 0 ? 'pax' : 'pax') : 'pax'}</option>)}
            </select>
          </div>
          ...
          {/* Submit */}
          <button type="submit" style={{ background: '#8BAA1D', border: 'none', cursor: 'pointer', padding: '0 1.5rem', transform: 'skewX(-12deg)', flexShrink: 0, display: 'flex', alignItems: 'center' }}>
            <svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="#ffffff" strokeWidth="2.5" style={{ transform: 'skewX(12deg)' }}>
