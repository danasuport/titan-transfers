    ...
    <section>
      ...
      <div style={{ display: 'flex', alignItems: 'center', gap: '1.5rem', marginBottom: '1.5rem', flexWrap: 'wrap' }}>
        ...
        <div style={{ transform: 'skewX(-8deg)', border: '1.5px solid #e5e7eb', background: '#ffffff', display: 'flex', alignItems: 'center', gap: '0.5rem', padding: '0.45rem 0.9rem', flex: 1, minWidth: '180px', maxWidth: '280px' }}>
          ...
          <input
            ...
            placeholder={pick(locale, { en: 'Filter destination...', es: 'Filtrar destino...', ar: 'تصفية الوجهة...', it: 'Filtra destinazione...', de: 'Ziel filtern...' })}
            style={{
              transform: 'skewX(8deg)',
        ...
        {filter && (
          <span style={{ fontSize: '0.78rem', color: '#64748b' }}>
            {filtered.length} {pick(locale, { en: 'results', es: 'resultados', ar: 'نتائج', it: 'risultati', de: 'Ergebnisse' })}
          </span>
        )}
      </div>
      ...
      {filtered.length === 0 && (
        <p style={{ color: '#64748b', fontSize: '0.875rem', marginTop: '1rem' }}>
          {pick(locale, { en: 'No results', es: 'Sin resultados', ar: 'لا توجد نتائج', it: 'Nessun risultato', de: 'Keine Ergebnisse' })}
        </p>
      )}
    </section>
  )
