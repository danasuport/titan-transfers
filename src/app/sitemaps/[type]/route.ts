...
function xml(urls: { loc: string; lastmod?: string; changefreq?: string; priority?: string; alternates?: { en: string; es: string; it?: string } }[]) {
  ...
  <url>
    ...
    <xhtml:link rel="alternate" hreflang="es" href="${u.alternates.es}"/>${u.alternates.it ? `
    <xhtml:link rel="alternate" hreflang="it" href="${u.alternates.it}"/>` : ''}
...
export async function GET(_req: Request, { params }: { params: Promise<{ type: string }> }) {
  ...
  if (type === 'pages.xml') {
    const staticPages = [
      { path: '/', espath: '/es/', itpath: '/it/', freq: 'daily', priority: '1.0' },
      { path: '/airport-transfers-private-taxi/', espath: '/es/traslados-aeropuerto-privados-taxi/', itpath: '/it/trasferimenti-aeroporto-taxi-privato/', freq: 'weekly', priority: '0.9' },
      { path: '/private-transfers/', espath: '/es/traslados-privados-taxi/', itpath: '/it/trasferimenti-privati-taxi/', freq: 'weekly', priority: '0.9' },
      { path: '/regions/', espath: '/es/regiones/', itpath: '/it/regioni/', freq: 'weekly', priority: '0.8' },
      { path: '/services/', espath: '/es/servicios/', itpath: '/it/servizi/', freq: 'weekly', priority: '0.8' },
      { path: '/blog/', espath: '/es/blog/', itpath: '/it/blog/', freq: 'daily', priority: '0.8' },
      { path: '/booking/', espath: '/es/reserva/', itpath: '/it/prenotazione/', freq: 'monthly', priority: '0.9' },
      { path: '/contact/', espath: '/es/contacto/', itpath: '/it/contatto/', freq: 'monthly', priority: '0.6' },
      { path: '/about/', espath: '/es/sobre-nosotros/', itpath: '/it/chi-siamo/', freq: 'monthly', priority: '0.6' },
      { path: '/faq/', espath: '/es/preguntas-frecuentes/', itpath: '/it/domande-frequenti/', freq: 'monthly', priority: '0.7' },
      { path: '/web-sitemap/', espath: '/es/mapa-del-sitio/', itpath: '/it/mappa-del-sito/', freq: 'monthly', priority: '0.5' },
      { path: '/privacy-policy/', espath: '/es/politica-de-privacidad/', itpath: '/it/informativa-privacy/', freq: 'yearly', priority: '0.3' },
      { path: '/terms-and-conditions/', espath: '/es/terminos-y-condiciones/', itpath: '/it/termini-e-condizioni/', freq: 'yearly', priority: '0.3' },
      { path: '/cookie-policy/', espath: '/es/politica-de-cookies/', itpath: '/it/informativa-cookie/', freq: 'yearly', priority: '0.3' },
      { path: '/legal-notice/', espath: '/es/aviso-legal/', itpath: '/it/note-legali/', freq: 'yearly', priority: '0.3' },
    ]
    body = xml(staticPages.map(p => ({
      loc: `${SITE_URL}${p.path}`,
      changefreq: p.freq,
      priority: p.priority,
      alternates: { en: `${SITE_URL}${p.path}`, es: `${SITE_URL}${p.espath}`, it: `${SITE_URL}${p.itpath}` },
    })))
  }
