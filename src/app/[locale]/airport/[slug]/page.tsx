...
import { getAirportUrl, getCountryUrl } from '@/lib/utils/slugHelpers'
...
export async function generateMetadata({ params }: { params: Promise<{ locale: string; slug: string }> }) {
  ...
  const currentPath = `${locale === 'en' ? '' : `/${locale}`}${getAirportUrl(airport, locale as Locale)}`
  return generatePageMetadata({
    ...
    alternates: [
      ...
      { locale: 'fr' as Locale, path: `/fr${getAirportUrl(airport, 'fr')}` },
    ],
  })
}
...
export default async function AirportPage({ params }: { params: Promise<{ locale: string; slug: string }> }) {
  ...
  const breadcrumbs = [
    ...(airport.country ? [{ label: airport.country.title || '', href: getCountryUrl(airport.country, locale as Locale) }] : []),
    { label: airportTitle },
  ]
  ...
  return (
    <>
      <SchemaOrg data={generateTaxiServiceSchema({ name: `${airportTitle} ${t('transfers')}`, description: t('transferBestPriceDesc', { airport: airportTitle }), url: getAirportUrl(airport, 'en'), areaServed: cityName, rating: 4.8, reviewCount: 2500 })} />
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: '1rem' }}>
              <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: '0.75rem' }}>
                {[
                  ...
                  { icon: <IconPlane />, title: pick(locale, { en: 'Professional driver', es: 'Conductor profesional', ar: 'سائق محترف', it: 'Autista professionale', de: 'Professioneller Fahrer', fr: 'Chauffeur professionnel' }), desc: pick(locale, { en: 'With name sign at arrivals', es: 'Con cartel con tu nombre', ar: 'يحمل لافتة باسمك عند الوصول', it: 'Con cartello con il tuo nome', de: 'Mit Namensschild bei der Ankunft', fr: 'Avec un panneau nominatif à l\'arrivée' }) },
                ].map((item, i) => (
        ...
        <section style={{ background: '#F8FAF0', padding: '5rem 6vw' }}>
          <RoutesList
            ...
            title={pick(locale, {
              ...
              fr: `Itinéraires populaires depuis ${airportTitle}`,
            })}
          />
        </section>
      )}
          ...
          <div className="resp-img-panel" style={{ position: 'relative', clipPath: 'polygon(0% 0%, 92% 0%, 100% 100%, 0% 100%)' }}>
            <Image
              ...
              alt={pick(locale, {
                ...
                fr: `Chauffeur professionnel ${airportTitle}`,
              })}
              fill
              style={{ objectFit: 'cover', objectPosition: 'center' }}
              sizes="50vw"
