...
export async function generateMetadata({ params }: { params: Promise<{ locale: string }> }) {
  ...
  return {
    alternates: staticPageAlternates('/terms/', locale),
    title: pick(locale, {
      en: 'Terms and Conditions | Titan Transfers',
      ...
      fr: 'Conditions générales | Titan Transfers',
    }),
    description: pick(locale, {
      ...
      fr: 'Conditions générales pour les services de transfert privé de Titan Transfers.',
    }),
...
function BodyAR() {
  return (
    <>
      <p>تنظم هذه الشروط والأحكام حجز خدمات النقل الخاص عبر <strong>https://titantransfers.com</strong>، التي تشغّلها Uep Tech And Solutions Fzco.</p>
      <h2>١. الحجوزات</h2>
      <p>يُعتبر الحجز مؤكداً بمجرد أن يتلقى العميل رسالة تأكيد بالبريد الإلكتروني. السعر المعروض عند الحجز هو السعر النهائي ويشمل جميع الضرائب المطبقة.</p>
      <h2>٢. السعر الثابت</h2>
      <p>تُقدم جميع رحلاتنا بسعر ثابت متفق عليه قبل السفر. لا توجد رسوم إضافية لحركة المرور أو التأخيرات المعتادة أو تغييرات المسار ضمن الرحلة المحجوزة.</p>
      <h2>٣. الإلغاءات والتعديلات</h2>
      <ul>
        <li><strong>إلغاء مجاني</strong> حتى ٢٤ ساعة قبل وقت الاستلام المحدد.</li>
        <li>قد يُحتسب الإلغاء قبل أقل من ٢٤ ساعة من الاستلام بالكامل.</li>
        <li>التعديلات على التاريخ أو الوقت أو الوجهة تخضع للتوفر وقد تؤدي إلى تعديل السعر.</li>
      </ul>
      <h2>٤. الالتزام بالمواعيد ووقت الانتظار</h2>
      <p>لاستلام المطار، سينتظر السائق ٦٠ دقيقة من وقت هبوط الرحلة الفعلي. لنقاط الاستلام الأخرى، وقت الانتظار القياسي هو ١٥ دقيقة.</p>
      <h2>٥. الأمتعة</h2>
      <p>يشمل كل حجز الأمتعة القياسية المبينة أثناء عملية الحجز. يجب الإبلاغ عن الأمتعة الزائدة مسبقاً وقد تتطلب تكلفة إضافية.</p>
      <h2>٦. المسؤولية</h2>
      <p>لا تتحمل تايتن ترانسفرز مسؤولية الخسائر الناجمة عن ظروف خارجة عن إرادتها (الطقس الشديد، إغلاق الطرق، الإضرابات، إلخ). في حالات القوة القاهرة، سيتم تقديم حل بديل أو استرداد كامل.</p>
      <h2>٧. الشكاوى</h2>
      <p>يجب تقديم أي شكوى خلال ٧ أيام من تنفيذ الخدمة عبر إرسال بريد إلكتروني إلى <strong>info@titantransfers.com</strong>.</p>
      <h2>٨. القانون الحاكم</h2>
      <p>تخضع هذه الشروط لقوانين الإمارات العربية المتحدة. تُحال أي نزاعات إلى المحاكم المختصة في دبي.</p>
    </>
  )
}
