...
export async function generateMetadata({ params }: { params: Promise<{ locale: string }> }) {
  ...
  return {
    title: pick(locale, {
      ...
      fr: 'Message envoyé | Titan Transfers',
    }),
...
export default async function ContactSentPage({ params }: { params: Promise<{ locale: string }> }) {
  ...
  return (
    <div className="site-container px-4 py-8 sm:px-6 lg:px-8">
      <Breadcrumbs items={[
        { label: pick(locale, { en: 'Contact', es: 'Contacto', ar: 'تواصل معنا', it: 'Contatto', de: 'Kontakt', fr: 'Contact' }), href: '/contact/' },
        { label: pick(locale, { en: 'Message sent', es: 'Mensaje enviado', ar: 'تم إرسال الرسالة', it: 'Messaggio inviato', de: 'Nachricht gesendet', fr: 'Message envoyé' }) },
      ]} />
      ...
      <div style={{ maxWidth: '640px', margin: '3rem auto', textAlign: 'center', padding: '2.5rem 1.5rem', background: '#F8FAF0', border: '1.5px solid #8BAA1D' }}>
        ...
        <h1 style={{ fontSize: 'clamp(1.5rem, 3vw, 2.25rem)', color: '#242426', fontWeight: 700, marginBottom: '0.75rem' }}>
          {pick(locale, { en: 'Message sent!', es: '¡Mensaje enviado!', ar: '!تم إرسال الرسالة', it: '!Messaggio inviato!', de: 'Nachricht gesendet!', fr: 'Message envoyé !' })}
        </h1>
        ...
        <p style={{ fontSize: '1rem', color: '#475569', lineHeight: 1.6, marginBottom: '2rem' }}>
          {pick(locale, {
            ...
            fr: "Merci de nous avoir contactés chez Titan Transfers. Nous reviendrons vers vous dès que possible, généralement dans les 24 heures.",
          })}
        </p>
        ...
        <Link href="/" style={{
          ...
          <span style={{ transform: 'skewX(12deg)', display: 'inline-flex', alignItems: 'center', gap: '0.5rem' }}>
            {pick(locale, { en: 'Back to home', es: 'Volver al inicio', ar: 'العودة إلى الرئيسية', it: 'Torna all\'inizio', de: 'Zurück zur Startseite', fr: 'Retour à l\'accueil' })}
